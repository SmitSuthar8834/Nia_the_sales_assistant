    ...
    for endpoint in endpoints_to_test:
        try:
            ...
            if 'conversion_analysis' in endpoint['url']:
                ...
                mock_path = 'ai_service.services.GeminiAIService.predict_deal_size_and_timeline'
                ...
                mock_path = 'ai_service.services.GeminiAIService.analyze_historical_patterns'
            else:
...
def test_comprehensive_opportunity_intelligence_endpoint():
    ...
    try:
        with patch.multiple('ai_service.services.GeminiAIService', **{
            ...
            if response.status_code == 200:
                ...
                if data.get('success'):
                    ...
                    print(f"  Components included: {list(data.get('intelligence_metadata', {}).get('components_included', {}).keys())}")
                    return True
                else:
                    print(f"✗ Comprehensive Opportunity Intelligence: API returned success=False")
                    print(f"  Error: {data.get('error', 'Unknown error')}")
                    return False
            else:
                print(f"✗ Comprehensive Opportunity Intelligence: HTTP {response.status_code}")
                print(f"  Response: {response.content.decode()}")
                return False
