...
def test_api_endpoints():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get('success') and data.get('lead_information'):
                ...
                print(f"    - Extracted company: {data['lead_information'].get('company_name', 'N/A')}")
                print(f"    - Contact: {data['lead_information'].get('contact_details', {}).get('name', 'N/A')}")
                print(f"    - Pain points: {len(data['lead_information'].get('pain_points', []))}")
            else:
    ...
    try:
        ...
        if response.status_code == 200:
            data = response.json()
            print(f"  ✅ Leads endpoint working (found {len(data.get('results', []))} leads)")
        else:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get('success'):
                ...
                print(f"    - Quality score: {recommendations.get('quality_score', 'N/A')}")
