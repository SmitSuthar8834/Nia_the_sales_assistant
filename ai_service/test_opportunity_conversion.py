...
from ai_service.models import AIInsights, Lead, Opportunity, OpportunityIntelligence
        ...
        mock_response.text = json.dumps(
            {
                ...
                "blocking_factors": ["Decision maker not yet identified"],
                "recommended_for_conversion": True,
...
class DealSizeTimelinePredictionTests(OpportunityConversionIntelligenceTestCase):
    ...
    def test_predict_deal_size_and_timeline(self, mock_api_call):
        ...
        mock_response.text = json.dumps(
            {
                ...
                "risk_factors": ["Budget approval process complexity"],
            }
        )
        mock_api_call.return_value = mock_response
        ...
        result = ai_service.predict_deal_size_and_timeline(
            self.sample_lead_data, self.sample_opportunity_data
        )
        ...
        self.assertIn(
            "Company size indicates mid-market budget capacity",
            result["deal_size_factors"],
        )
        self.assertIn(
            "Urgent business need creates timeline pressure",
            result["accelerating_factors"],
        )
    ...
    def test_deal_prediction_validation(self):
        ...
        validated = ai_service._validate_deal_predictions(
            invalid_data, self.sample_lead_data
        )

        # Check that validation corrected the values
        deal_size = validated["deal_size_prediction"]
        self.assertGreaterEqual(
            deal_size["minimum_value"], 1000
        )  # Should be corrected to minimum
    ...
    def test_deal_size_timeline_prediction_api_endpoint(self):
        ...
        with patch(
            ...
            self.assertTrue(data["success"])
            self.assertEqual(
                data["predictions"]["deal_size_prediction"]["most_likely_value"], 50000
            )
            self.assertEqual(
                data["predictions"]["timeline_prediction"]["most_likely_days"], 120
            )
...
class SalesStageRecommendationTests(OpportunityConversionIntelligenceTestCase):
    ...
    def test_recommend_sales_stage(self, mock_api_call):
        ...
        mock_response.text = json.dumps(
            {
                ...
                "stage_risks": ["Budget approval process may be complex"],
                "success_metrics": ["Budget range confirmed within 2 weeks"],
            }
        )
        mock_api_call.return_value = mock_response
        ...
        result = ai_service.recommend_sales_stage(
            self.sample_lead_data, self.sample_opportunity_data, "prospecting"
        )
        ...
        self.assertIn(
            "Budget authority not confirmed", result["stage_requirements_missing"]
        )
        self.assertIn(
            "Schedule budget qualification call", result["advancement_actions"]
        )
    ...
    def test_sales_stage_recommendation_api_endpoint(self):
        ...
        with patch(
            ...
            self.assertTrue(data["success"])
            self.assertEqual(
                data["stage_recommendations"]["current_stage_assessment"][
                    "recommended_stage"
                ],
                "qualification",
            )
...
class RiskFactorAnalysisTests(OpportunityConversionIntelligenceTestCase):
    ...
    def test_identify_risk_factors_and_mitigation(self, mock_api_call):
        ...
        mock_response.text = json.dumps(
            {
                ...
                "early_warning_indicators": ["Delayed responses to proposals"],
            }
        )
        mock_api_call.return_value = mock_response
        ...
        self.assertIn(
            "Emphasize unique differentiators early in process",
            competitive_strategy["strategies"],
        )
    ...
    def test_risk_factor_analysis_api_endpoint(self):
        ...
        with patch(
            ...
            self.assertTrue(data["success"])
            self.assertEqual(
                data["risk_analysis"]["overall_risk_assessment"]["risk_level"], "medium"
            )
...
class HistoricalPatternAnalysisTests(OpportunityConversionIntelligenceTestCase):
    ...
    def test_analyze_historical_patterns(self, mock_api_call):
        ...
        result = ai_service.analyze_historical_patterns(
            self.sample_lead_data, str(self.user.id)
        )
    ...
    def test_historical_pattern_analysis_api_endpoint(self):
        ...
        with patch(
            ...
            response = self.client.post(
                url,
                {"lead_data": self.sample_lead_data, "user_id": str(self.user.id)},
                content_type="application/json",
            )
            ...
            self.assertTrue(data["success"])
            self.assertEqual(
                data["historical_analysis"]["similar_leads_analysis"][
                    "similar_leads_count"
                ],
                25,
            )
...
class ComprehensiveOpportunityIntelligenceTests(
    OpportunityConversionIntelligenceTestCase
):
...
class OpportunityConversionAccuracyTests(OpportunityConversionIntelligenceTestCase):
    ...
    def test_conversion_probability_accuracy_validation(self):
        ...
        test_cases = [
            {
                ...
                "expected": 0,
            },  # Negative should be corrected to 0
            {
                "conversion_probability": 150,
                "expected": 100,
            },  # Over 100 should be corrected to 100
            {
                "conversion_probability": 75,
                "expected": 75,
        ...
        for case in test_cases:
            data = {
                "conversion_probability": case["conversion_probability"],
                "conversion_confidence": 80,
            }
            validated = ai_service._validate_conversion_analysis(
                data, self.sample_lead_data
            )
    ...
    def test_deal_size_prediction_accuracy_validation(self):
        ...
        self.assertGreaterEqual(
            deal_size["most_likely_value"], deal_size["minimum_value"]
        )
        self.assertLessEqual(deal_size["most_likely_value"], deal_size["maximum_value"])
    ...
    def test_historical_pattern_accuracy_metrics(self):
        ...
        validated = ai_service._validate_historical_analysis(
            invalid_historical, self.sample_lead_data
        )
