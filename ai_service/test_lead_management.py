...
class LeadModelTest(TestCase):
    ...
    def test_lead_str_representation(self):
        ...
        lead = Lead.objects.create(
            user=self.user, company_name="Test Company", status=Lead.Status.QUALIFIED
        )
...
class AIInsightsModelTest(TestCase):
    ...
    def test_ai_insights_str_representation(self):
        ...
        insights = AIInsights.objects.create(lead=self.lead, lead_score=75.0)
...
class LeadSerializerTest(TestCase):
    ...
    def test_lead_create_serializer(self):
        ...
        create_data["conversation_text"] = (
            "Customer mentioned they need automation solutions..."
        )
...
class LeadAPITest(APITestCase):
    ...
    def test_list_leads(self):
        ...
        # Create test leads
        Lead.objects.create(
            user=self.user, company_name="Company A", status=Lead.Status.NEW
        )
        Lead.objects.create(
            user=self.user, company_name="Company B", status=Lead.Status.QUALIFIED
        )
    ...
    def test_search_leads(self):
        ...
        Lead.objects.create(
            user=self.user, company_name="Tech Solutions Inc", status=Lead.Status.NEW
        )
        Lead.objects.create(
            user=self.user, company_name="Marketing Agency", status=Lead.Status.NEW
        )
        ...
        self.assertEqual(len(response.data["results"]), 1)
        self.assertEqual(
            response.data["results"][0]["company_name"], "Tech Solutions Inc"
        )
    ...
    def test_update_lead(self):
        ...
        lead = Lead.objects.create(
            user=self.user, company_name="Test Company", status=Lead.Status.NEW
        )

        update_data = {"status": "qualified", "industry": "Healthcare"}
    ...
    def test_delete_lead(self):
        ...
        lead = Lead.objects.create(user=self.user, company_name="Test Company")
    ...
    def test_get_high_priority_leads(self):
        ...
        self.assertEqual(len(response.data["results"]), 1)
        self.assertEqual(
            response.data["results"][0]["company_name"], "Priority Company"
        )
    ...
    def test_lead_analytics(self):
        ...
        AIInsights.objects.create(
            lead=lead1, lead_score=85.0, quality_tier=AIInsights.QualityTier.HIGH
        )
        ...
        AIInsights.objects.create(
            lead=lead2, lead_score=60.0, quality_tier=AIInsights.QualityTier.MEDIUM
        )
...
class LeadTaskTest(TestCase):
    ...
    def test_analyze_lead_with_ai_no_conversation(self):
        ...
        lead_no_conversation = Lead.objects.create(
            user=self.user, company_name="No Conversation Company"
        )
