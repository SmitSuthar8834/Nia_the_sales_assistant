...
class LeadQualityScoringTests(TestCase):
    ...
    def test_calculate_lead_quality_score_minimal_data(self):
        ...
        with patch('ai_service.services.genai.GenerativeModel') as mock_model:
            ...
            self.assertIn(result['quality_tier'], ['low', 'medium'])  # Allow for some variation in AI response
...
class SalesStrategyGenerationTests(TestCase):
    ...
    def test_generate_sales_strategy_success(self, mock_model):
        ...
        result = self.ai_service.generate_sales_strategy(self.sample_lead_data, self.sample_quality_score)
    ...
    def test_generate_sales_strategy_different_tiers(self):
        ...
        for tier in quality_tiers:
            quality_score = {"quality_tier": tier, "overall_score": 80 if tier == 'high' else 50}
            
            with patch('ai_service.services.genai.GenerativeModel') as mock_model:
                mock_response = MagicMock()
                mock_response.text = json.dumps({
                    "primary_strategy": "relationship" if tier == 'high' else "consultative",
                    "approach_rationale": f"Strategy tailored for {tier} quality lead"
                })
                mock_model.return_value.generate_content.return_value = mock_response
                
                result = self.ai_service.generate_sales_strategy(self.sample_lead_data, quality_score)
                
                self.assertIn('strategy_metadata', result)
                self.assertEqual(result['strategy_metadata']['based_on_quality_tier'], tier)
...
class RecommendationConsistencyTests(TestCase):
    ...
    def test_recommendation_ranking_consistency(self):
        ...
        with patch('ai_service.services.genai.GenerativeModel') as mock_model:
            ...
            mock_response.text = json.dumps({
                "recommendations": [
                    {"type": "follow_up", "priority": "low", "title": "Low priority item"},
                    {"type": "next_step", "priority": "high", "title": "High priority item"},
                    {"type": "strategy", "priority": "medium", "title": "Medium priority item"}
                ]
            })
            mock_model.return_value.generate_content.return_value = mock_response
            ...
            if 'recommendations' in result and len(result['recommendations']) > 1:
                ...
                priorities = [rec.get('priority', 'low') for rec in result['recommendations']]
    ...
    def test_recommendation_structure_validation(self):
        ...
        with patch('ai_service.services.genai.GenerativeModel') as mock_model:
            ...
            if result['recommendations']:
                ...
                required_fields = ['type', 'title', 'description', 'priority', 'timeline']
