...
from .chat_models import ChatAnalytics, ChatBotCommand, ChatMessage, ChatSession
...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def handle_voice_transition_request(self, data):
        ...
        try:
            ...
            await self.send(
                text_data=json.dumps(
                    {"type": "error", "message": "Failed to initiate voice call"}
                )
            )
    ...
    def create_chat_message(
        ...
        last_message = (
            ChatMessage.objects.filter(session=self.chat_session)
    ...
    def update_session_activity(self, typing=False):
        ...
        self.chat_session.save(
            update_fields=["last_activity", "typing_indicator_active"]
        )
    ...
    def mark_messages_read(self, message_ids):
        ...
        ChatMessage.objects.filter(
            id__in=message_ids, session=self.chat_session
        ).update(status=ChatMessage.Status.READ, read_at=timezone.now())
    ...
    def get_recent_messages(self, limit=50):
        ...
        messages = ChatMessage.objects.filter(session=self.chat_session).order_by(
            "-message_number"
        )[:limit]
    ...
    async def send_bot_commands(self):
        ...
        try:
            ...
            await self.send(
                text_data=json.dumps({"type": "bot_commands", "commands": commands})
            )
        except Exception as e:
    ...
    async def cmd_help(self):
        ...
        return {"content": help_text, "metadata": {"command_type": "help"}}
    ...
    async def process_message_ai(self, message):
        ...
        try:
            ...
            logger.info(
                f"AI processing message {message.id} - content: {message.content[:100]}..."
            )
        except Exception as e:
    ...
    async def generate_nia_response(self, user_message):
        ...
        try:
            ...
            if any(word in content for word in ["hello", "hi", "hey"]):
                response_content = (
                    "Hello! I'm NIA, your AI sales assistant. How can I help you today?"
                )
    ...
    def search_chat_history(self, query):
        ...
        search_results = ChatSearchHistory.objects.filter(
            user=self.user, searchable_content__icontains=query
        )[:10]
