...
class ConversationSummaryService:
    ...
    async def generate_conversation_summary(self, session_id: str) -> Dict:
        ...
        try:
            ...
            @sync_to_async
            def get_turns():
                return list(ConversationTurn.objects.filter(session=session).order_by('turn_number'))
            ...
            for turn in turns:
                speaker = "Sales Rep" if turn.speaker == ConversationTurn.Speaker.USER else "NIA"
            ...
            if self.model:
                ...
                summary_data = await self._generate_basic_summary(conversation_text, turns)
    ...
    async def _generate_basic_summary(self, conversation_text: str, turns) -> Dict:
        ...
        try:
            ...
            import re
            company_matches = re.findall(r'\b[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*\b', conversation_text)
            
            # Extract potential contact information
            email_matches = re.findall(r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b', conversation_text)
            phone_matches = re.findall(r'\b\d{3}[-.]?\d{3}[-.]?\d{4}\b', conversation_text)
            
            # Basic keyword extraction for pain points
            pain_keywords = ['problem', 'issue', 'challenge', 'difficulty', 'struggle', 'pain', 'concern']
    ...
    def update_context(self, session_id: str, updates: Dict) -> Dict:
        ...
        for key, value in updates.items():
            if key == 'extracted_entities':
                ...
                context['conversation_flow'].extend(value if isinstance(value, list) else [value])
            elif key == 'context_history':
                context['context_history'].extend(value if isinstance(value, list) else [value])
            elif key == 'conversation_summary':
                for summary_key, summary_value in value.items():
                    if isinstance(context['conversation_summary'].get(summary_key), list):
                        context['conversation_summary'][summary_key].extend(
                            summary_value if isinstance(summary_value, list) else [summary_value]
                        )
                    else:
...
class VoiceProcessingService:
    ...
    async def initiate_call(self, user_id: str, caller_id: str = "") -> CallSession:
        ...
        try:
            ...
            logger.info(f"Initiated call session {session.session_id} for user {user.username}")
            return session
    ...
    async def _process_audio_track(self, session_id: str, track):
        ...
        try:
            ...
            async for frame in track:
                ...
                asyncio.create_task(self._transcribe_audio_chunk(session_id, chunk_number))
    ...
    async def process_audio_stream(self, session_id: str, audio_data: bytes) -> dict:
        ...
        try:
            ...
            try:
                ...
                voice_config = await VoiceConfiguration.objects.acreate(user=session.user)
    ...
    async def end_call(self, session_id: str) -> dict:
        ...
        try:
            ...
            # Generate comprehensive conversation summary
            conversation_summary = await self.summary_service.generate_conversation_summary(session_id)
            ...
            audio_chunks = await AudioChunk.objects.filter(
                session=session
            ).acount()
...
class AudioFileStorageService:
    ...
    async def store_audio_file(self, session_id: str, audio_data: bytes, 
                              file_type: str = 'wav', metadata: Dict = None) -> str:
...
class AudioBufferManager:
    ...
    async def get_buffered_audio(self, session_id: str, chunk_size: int = None) -> bytes:
