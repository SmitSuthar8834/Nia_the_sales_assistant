...
from .services import audio_buffer_manager, voice_service
...
class VoiceConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        try:
            ...
            await self.channel_layer.group_add(
                f"voice_session_{self.session_id}", self.channel_name
            )
    ...
    async def _handle_audio_data(self, audio_data):
        ...
        try:
            ...
            await self.send(
                text_data=json.dumps({"type": "transcription", "data": result})
            )
            ...
            await self.send(
                text_data=json.dumps(
                    {"type": "error", "message": f"Audio processing error: {str(e)}"}
                )
            )
    ...
    async def _generate_speech_response(self, text):
        ...
        try:
            ...
            await self.send(
                text_data=json.dumps(
                    {"type": "error", "message": f"Speech generation error: {str(e)}"}
                )
            )
    ...
    async def _end_call(self):
        ...
        try:
            ...
            # Send call summary
            await self.send(
                text_data=json.dumps({"type": "call_ended", "summary": summary})
            )
            ...
            await self.send(
                text_data=json.dumps(
                    {"type": "error", "message": f"Error ending call: {str(e)}"}
                )
            )
    ...
    async def voice_message(self, event):
        ...
        await self.send(
            text_data=json.dumps({"type": "voice_message", "data": event["data"]})
        )
    ...
    async def transcription_update(self, event):
        ...
        await self.send(
            text_data=json.dumps(
                {"type": "transcription_update", "data": event["data"]}
            )
        )
