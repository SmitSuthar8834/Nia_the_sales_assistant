...
def initiate_call(request):
    ...
    try:
        ...
        context = voice_service.context_manager.initialize_context(
            str(session.session_id), str(request.user.id)
        )
...
def setup_webrtc(request):
    ...
    try:
        ...
        try:
            ...
            return Response(
                {"answer": answer, "message": "WebRTC connection established"},
...
def process_audio(request):
    ...
    try:
        ...
        if not audio_data:
            return Response(
                {"error": "audio file is required"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    try:
        ...
        try:
            ...
            response["Content-Disposition"] = (
                f'attachment; filename="response_{session_id}.wav"'
            )
            return response
...
def end_call(request):
    ...
    try:
        ...
        from .models import AudioChunk, ConversationTurn
...
def user_call_sessions(request):
    ...
    try:
        sessions = CallSession.objects.filter(user=request.user).order_by(
            "-start_time"
        )[:20]
        ...
        return Response(
            {"sessions": sessions_data, "count": len(sessions_data)},
            status=status.HTTP_200_OK,
        )
