...
from .models import CallSession, ConversationTurn
...
class VoiceProcessingIntegrationTest(TransactionTestCase):
    ...
    async def test_complete_voice_call_workflow(
        self, mock_tts_client, mock_speech_client
    ):
        ...
        mock_recognition_result.alternatives[0].transcript = (
            "Hello, I have a lead from Acme Corp"
        )
        ...
        context = self.voice_service.context_manager.get_context(
        ...
        result = await self.voice_service.process_audio_stream(
            str(session.session_id), test_audio
        )
        ...
        updated_context = self.voice_service.context_manager.get_context(
            str(session.session_id)
        )
    ...
    async def test_conversation_summary_generation(self, mock_genai):
        ...
        session = await CallSession.objects.acreate(
            user=self.user, status=CallSession.Status.ENDED
        )
        ...
        self.assertEqual(
            summary["lead_information"]["company_name"], "Acme Corporation"
        )
    ...
    async def test_audio_file_storage_and_retrieval(self):
        ...
        file_path = await storage_service.store_audio_file(
            session_id, test_audio, "wav", {"test": "metadata"}
        )
...
class VoiceAPIIntegrationTest(APITestCase):
    ...
    def test_complete_api_workflow(self):
        ...
        response = self.client.post(
            "/api/voice/initiate/", {"caller_id": "+1234567890"}
        )
        ...
        # 4. End call
        response = self.client.post("/api/voice/end/", {"session_id": session_id})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_voice_configuration_management(self):
        ...
        response = self.client.put(
            "/api/voice/config/",
            {"language_code": "es-ES", "speaking_rate": 1.2, "pitch": 0.5},
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class VoiceWebSocketIntegrationTest(TransactionTestCase):
    ...
    async def test_websocket_voice_processing(self):
        ...
        communicator = WebsocketCommunicator(
            VoiceConsumer.as_asgi(), f"/ws/voice/{session.session_id}/"
        )
