...
def get_chat_sessions(request):
    ...
    try:
        sessions = ChatSession.objects.filter(
            user=request.user
        ).select_related('voice_session').prefetch_related('analytics')
        ...
        for session in sessions:
            session_data = {
                ...
                'voice_session_id': str(session.voice_session.session_id) if session.voice_session else None,
                'last_activity': session.last_activity.isoformat(),
                'message_count': session.analytics.total_messages if hasattr(session, 'analytics') else 0
            }
...
def get_chat_session(request, session_id):
    ...
    try:
        ...
        # Get recent messages
        messages = ChatMessage.objects.filter(session=session).order_by('message_number')[:50]
        messages_data = [
            {
                'id': str(msg.id),
        ...
        session_data = {
            ...
            'analytics': {
                ...
                'session_duration': str(session.analytics.session_duration) if session.analytics.session_duration else None
            } if hasattr(session, 'analytics') else None
        }
...
def upload_chat_file(request, session_id):
    ...
    try:
        ...
        last_message = ChatMessage.objects.filter(session=session).order_by('-message_number').first()
