...
def test_lead_extraction():
    ...
    try:
        ...
        print(f"Contact: {result.get('contact_details', {}).get('name', 'Not extracted')}")
        print(f"Email: {result.get('contact_details', {}).get('email', 'Not extracted')}")
        print(f"Budget: {result.get('budget_info', 'Not extracted')}")
        print(f"Timeline: {result.get('timeline', 'Not extracted')}")
        print(f"Confidence Score: {result.get('extraction_metadata', {}).get('confidence_score', 0)}")
        print(f"Data Completeness: {result.get('extraction_metadata', {}).get('data_completeness', 0)}%")
