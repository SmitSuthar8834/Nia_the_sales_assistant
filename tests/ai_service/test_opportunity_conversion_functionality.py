...
def test_opportunity_conversion_models():
    ...
    try:
        ...
        user, created = User.objects.get_or_create(
            username="test_conversion_user", defaults={"email": "test@conversion.com"}
        )
        ...
        print(
            f"✓ Created/Retrieved AI Insights with conversion score: {ai_insights.opportunity_conversion_score}"
        )
        print(
            f"✓ Created/Retrieved Opportunity: {opportunity.name} (${opportunity.estimated_value})"
        )
        print(
            f"✓ Created/Retrieved Intelligence: {intelligence.conversion_probability}% probability"
        )

        # Test model properties
        print(
            f"✓ Lead should convert to opportunity: {ai_insights.should_convert_to_opportunity}"
        )
...
def test_opportunity_conversion_services():
    ...
    try:
        ...
        print(
            f"✓ Default conversion analysis: {conversion_analysis['conversion_probability']}% probability"
        )

        # Test deal predictions default
        deal_predictions = ai_service._get_default_deal_predictions()
        print(
            f"✓ Default deal predictions: ${deal_predictions['deal_size_prediction']['most_likely_value']}"
        )

        # Test stage recommendations default
        stage_recommendations = ai_service._get_default_stage_recommendations()
        print(
            f"✓ Default stage recommendation: {stage_recommendations['current_stage_assessment']['recommended_stage']}"
        )

        # Test risk analysis default
        risk_analysis = ai_service._get_default_risk_analysis()
        print(
            f"✓ Default risk analysis: {risk_analysis['overall_risk_assessment']['risk_level']} risk"
        )

        # Test historical analysis default
        historical_analysis = ai_service._get_default_historical_analysis()
        print(
            f"✓ Default historical analysis: {historical_analysis['similar_leads_analysis']['average_conversion_rate']}% avg conversion"
        )
        ...
        validated_conversion = ai_service._validate_conversion_analysis(
            invalid_conversion, lead_data
        )
        print(
            f"✓ Conversion validation: {validated_conversion['conversion_probability']}% (corrected from 150%)"
        )
        ...
        validated_predictions = ai_service._validate_deal_predictions(
            invalid_predictions, lead_data
        )
        deal_size = validated_predictions["deal_size_prediction"]
        print(
            f"✓ Deal size validation: ${deal_size['minimum_value']} - ${deal_size['maximum_value']} (corrected)"
        )
...
def test_opportunity_conversion_serializers():
    ...
    try:
        ...
        print(
            f"✓ Opportunity serializer: {opp_data['name']} - ${opp_data['estimated_value']}"
        )
        ...
        if hasattr(opportunity, "intelligence"):
            intel_serializer = OpportunityIntelligenceSerializer(
                opportunity.intelligence
            )
            intel_data = intel_serializer.data
            print(
                f"✓ Intelligence serializer: {intel_data['conversion_probability']}% probability"
            )
