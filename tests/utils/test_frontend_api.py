    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get("success") and data.get("lead_information"):
                ...
                print(
                    f"   ✅ Analysis working - Company: {lead_info.get('company_name', 'N/A')}"
                )
                print(
                    f"      Contact: {lead_info.get('contact_details', {}).get('name', 'N/A')}"
                )
                print(f"      Pain points: {len(lead_info.get('pain_points', []))}")
                print(
                    f"      Confidence: {lead_info.get('extraction_metadata', {}).get('confidence_score', 'N/A')}%"
                )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            lead_data = {
                ...
                "contact_info": {"name": "Test Contact", "email": "test@example.com"},
                "source": "api_test",
            ...
            if create_response.status_code == 201:
                ...
                print(
                    f"   ✅ Lead creation working - ID: {create_data.get('lead', {}).get('id', 'N/A')}"
                )
                tests.append(("Lead Management", True))
            else:
                print(
                    f"   ⚠️  Lead retrieval works but creation failed: {create_response.status_code}"
                )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get("success") and data.get("recommendations"):
                ...
                print(
                    f"      Conversion Probability: {rec.get('conversion_probability', 'N/A')}%"
                )
