...
def test_voice_service_basic_functionality():
    ...
    response = client.post(
        "/api/voice/initiate/", {"caller_id": "555-0123"}, format="json"
    )
    ...
    if response.status_code == 200:
        ...
        response = client.put(
            "/api/voice/config/",
            {"language_code": "es-ES", "speaking_rate": 1.2},
            format="json",
        )
    ...
    response = client.post("/api/voice/end/", {"session_id": session_id}, format="json")

    if response.status_code == 200:
        summary_data = response.json()
        print(
            f"✓ Call ended successfully. Duration: {summary_data.get('duration', 'N/A')}"
        )
    else:
...
def test_voice_service_models():
    ...
    session = CallSession.objects.create(
        user=user, caller_id="555-0456", status=CallSession.Status.ACTIVE
    )
    ...
    config, created = VoiceConfiguration.objects.get_or_create(
        user=user, defaults={"language_code": "en-US", "voice_name": "en-US-Wavenet-D"}
    )
    print(
        f"✓ VoiceConfiguration {'created' if created else 'retrieved'}: {config.language_code}"
    )
