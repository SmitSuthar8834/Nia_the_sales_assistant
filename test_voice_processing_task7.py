...
from voice_service.models import CallSession, ConversationTurn, VoiceConfiguration
...
async def test_conversation_context_management():
    ...
    print(f"✅ Updated context with entities: {len(updated_context['extracted_entities'])} types")
    ...
    print(f"✅ Added conversation flow: {len(final_context['conversation_flow'])} turns")
...
async def test_conversation_summary_generation():
    ...
    turns_data = [
        ...
        ("They have 50 employees and need a CRM solution. Budget is around $10,000.", "user"),
        ("That sounds promising. What's their timeline for implementation?", "nia"),
        ("They want to start within 3 months. The contact is John Smith at john@acme.com.", "user")
    ]
    ...
    for i, (content, speaker) in enumerate(turns_data):
        await ConversationTurn.objects.acreate(
            session=session,
            turn_number=i + 1,
            speaker=ConversationTurn.Speaker.USER if speaker == "user" else ConversationTurn.Speaker.NIA,
            content=content
        )
    ...
    print(f"✅ Extracted lead info: {summary['lead_information'].get('company_name', 'N/A')}")
...
async def test_voice_processing_integration():
    ...
    user, created = await sync_to_async(User.objects.get_or_create)(
        username='voiceuser',
        defaults={'email': 'voice@example.com'}
    )
    ...
    for i, content in enumerate(test_turns):
        ...
        voice_service.context_manager.update_context(str(session.session_id), context_updates)
    ...
    print(f"   - Conversion Probability: {summary.get('conversion_probability', 'N/A')}")
