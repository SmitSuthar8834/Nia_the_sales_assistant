...
from .models import (
    MeetingSession, MeetingParticipant, MeetingStatusUpdate,
    GoogleMeetCredentials, MicrosoftTeamsCredentials
)
...
try:
    from ai_service.models import Lead, AIInsights
...
class NIAMeetingScheduler:
    ...
    def get_available_time_slots(self, user: User, date_range: Tuple[datetime, datetime], 
                               meeting_type: NIAMeetingType = NIAMeetingType.GENERAL_CONSULTATION) -> List[Dict]:
            ...
            for slot in availability.get('available_slots', []):
                ...
                if slot_duration >= duration:
                    ...
                    available_slots.append({
                        ...
                        'preparation_time': self._calculate_preparation_time(meeting_type)
                    })
            
            # Sort by recommendation score
            available_slots.sort(key=lambda x: (x['recommended'], x['start_time']), reverse=True)
    ...
    def schedule_nia_meeting(self, user: User, meeting_data: Dict) -> Optional[MeetingSession]:
        """Schedule a meeting with NIA"""
        try:
            meeting_type = NIAMeetingType(meeting_data.get('meeting_type', NIAMeetingType.GENERAL_CONSULTATION.value))
            start_time = meeting_data['start_time']
            platform = meeting_data.get('platform', 'google_meet')  # Default to Google Meet
            ...
            description = self._generate_meeting_description(meeting_type, user, lead_id)
            ...
            if meeting_session:
                ...
                agenda = self._generate_meeting_agenda(meeting_session, meeting_type, lead_id)
    ...
    def _generate_meeting_title(self, meeting_type: NIAMeetingType, user: User, lead_id: str = None) -> str:
    ...
    def _generate_meeting_description(self, meeting_type: NIAMeetingType, user: User, lead_id: str = None) -> str:
        ...
        description = descriptions.get(meeting_type, "Meeting with NIA for sales assistance.")
    ...
    def _generate_meeting_agenda(self, meeting: MeetingSession, meeting_type: NIAMeetingType, lead_id: str = None) -> str:
            ...
            if meeting_type == NIAMeetingType.LEAD_CONSULTATION:
                ...
                if lead_id and AI_SERVICE_AVAILABLE:
                    try:
                        ...
                        if insights:
                            agenda_items.insert(2, f"AI insights review - Lead score: {insights.lead_score}/10 (5 min)")
                            if insights.recommended_actions:
                                agenda_items.insert(-1, "Review AI-recommended actions (5 min)")
                    except:
                        pass
    ...
    def _schedule_preparation_reminder(self, meeting: MeetingSession, meeting_type: NIAMeetingType):
    ...
    def generate_meeting_summary(self, meeting: MeetingSession) -> Dict:
        ...
        try:
            ...
            if nia_updates and nia_updates.metadata:
                meeting_type = NIAMeetingType(nia_updates.metadata.get('nia_meeting_type', 'general_consultation'))
            ...
            summary = {
                ...
                'meeting_type': meeting_type.value,
                'title': meeting.title,
                'duration_minutes': meeting.actual_duration_minutes or meeting.duration_minutes,
                'participants': meeting.participants.count(),
                'status': meeting.status,
                'key_outcomes': self._generate_key_outcomes(meeting, meeting_type, lead_id),
                'action_items': self._generate_action_items(meeting, meeting_type, lead_id),
                'next_steps': self._generate_next_steps(meeting, meeting_type, lead_id),
                'nia_recommendations': self._generate_nia_recommendations(meeting, meeting_type, lead_id),
                'follow_up_required': self._determine_follow_up_needed(meeting, meeting_type),
    ...
    def _generate_key_outcomes(self, meeting: MeetingSession, meeting_type: NIAMeetingType, lead_id: str = None) -> List[str]:
        ...
        return outcomes_templates.get(meeting_type, ["Meeting objectives discussed", "Action items identified"])
    
    def _generate_action_items(self, meeting: MeetingSession, meeting_type: NIAMeetingType, lead_id: str = None) -> List[Dict]:
        """Generate action items based on meeting context"""
        # Template-based action items
        action_templates = {
            NIAMeetingType.LEAD_CONSULTATION: [
                {"task": "Follow up with lead within 24 hours", "owner": "User", "due_date": "tomorrow"},
                {"task": "Prepare customized proposal", "owner": "User", "due_date": "3 days"},
                {"task": "Schedule product demo", "owner": "User", "due_date": "1 week"}
            ],
            NIAMeetingType.SALES_STRATEGY: [
                {"task": "Implement recommended pipeline changes", "owner": "User", "due_date": "1 week"},
                {"task": "Update CRM with new strategy", "owner": "User", "due_date": "2 days"},
                {"task": "Schedule follow-up strategy review", "owner": "NIA", "due_date": "2 weeks"}
            ]
        }
        
        return action_templates.get(meeting_type, [])
    
    def _generate_next_steps(self, meeting: MeetingSession, meeting_type: NIAMeetingType, lead_id: str = None) -> List[str]:
        ...
        return next_steps_templates.get(meeting_type, ["Continue with planned activities"])
    
    def _generate_nia_recommendations(self, meeting: MeetingSession, meeting_type: NIAMeetingType, lead_id: str = None) -> List[str]:
    ...
    def _determine_follow_up_needed(self, meeting: MeetingSession, meeting_type: NIAMeetingType) -> bool:
    ...
    def _calculate_meeting_effectiveness(self, meeting: MeetingSession) -> float:
        ...
        if meeting.actual_start_time and meeting.scheduled_start_time:
            delay_minutes = (meeting.actual_start_time - meeting.scheduled_start_time).total_seconds() / 60
            if delay_minutes <= 5:
                score += 1
    ...
    def get_meeting_analytics(self, user: User, date_range: Tuple[datetime, datetime] = None) -> Dict:
        ...
        try:
            ...
            completed_meetings = nia_meetings.filter(status=MeetingSession.Status.ENDED).count()
            cancelled_meetings = nia_meetings.filter(status=MeetingSession.Status.CANCELLED).count()
            ...
            for meeting in nia_meetings:
                ...
                if nia_update and nia_update.metadata:
                    meeting_type = nia_update.metadata.get('nia_meeting_type', 'general_consultation')
            ...
            return {
                ...
                'roi_indicators': {
                    'meetings_per_week': total_meetings / 4 if total_meetings > 0 else 0,
                    'avg_effectiveness': avg_effectiveness,
                    'consistency_score': self._calculate_consistency_score(nia_meetings)
                }
            }
    ...
    def _calculate_consistency_score(self, meetings) -> float:
        ...
        meeting_dates = [m.scheduled_start_time for m in meetings.order_by('scheduled_start_time')]
        gaps = []
