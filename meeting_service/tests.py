...
class MeetingParticipantTestCase(TestCase):
    ...
    def test_create_participant(self):
        ...
        self.assertEqual(
            participant.status, MeetingParticipant.ParticipantStatus.INVITED
        )
...
class MeetingStatusUpdateTestCase(TestCase):
    ...
    def test_create_status_update(self):
        ...
        self.assertEqual(
            status_update.update_type, MeetingStatusUpdate.UpdateType.CREATED
        )
...
class MicrosoftTeamsCredentialsTestCase(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(
            username="teamsuser", email="teams@example.com", password="testpass123"
        )
...
class TeamsMeetingSessionTestCase(TestCase):
    ...
    def test_create_teams_meeting_session(self):
        ...
        self.assertEqual(
            meeting.meeting_type, MeetingSession.MeetingType.MICROSOFT_TEAMS
        )
...
class MicrosoftTeamsServiceTestCase(TestCase):
    ...
    def test_create_teams_meeting_mock(self, mock_post):
        ...
        mock_response.json.return_value = {
            ...
            "organizer": {"emailAddress": {"address": "teamsservicetest@example.com"}},
        }
    ...
    def test_get_user_teams_meetings(self):
        ...
        self.assertEqual(
            teams_meetings[0].meeting_type, MeetingSession.MeetingType.MICROSOFT_TEAMS
        )
...
class UnifiedMeetingInterfaceTestCase(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(
            username="unifieduser", email="unified@example.com", password="testpass123"
        )
    ...
    def test_meeting_type_selection(self):
        ...
        self.assertEqual(
            google_meeting.meeting_type, MeetingSession.MeetingType.GOOGLE_MEET
        )
        self.assertEqual(
            teams_meeting.meeting_type, MeetingSession.MeetingType.MICROSOFT_TEAMS
        )
    ...
    def test_unified_meeting_dashboard_data(self):
        ...
        google_today = MeetingSession.objects.create(
            ...
            title="Google Today",
            meeting_type=MeetingSession.MeetingType.GOOGLE_MEET,
            scheduled_start_time=now.replace(
        ...
        today_meetings = [
            m for m in all_meetings if m.scheduled_start_time.date() == now.date()
        ]
...
class MeetingIntegrationTestCase(TestCase):
    ...
    def test_meeting_participant_workflow(self):
        ...
        self.assertEqual(
            organizer_participant.role, MeetingParticipant.ParticipantRole.ORGANIZER
        )
        self.assertEqual(
            organizer_participant.status, MeetingParticipant.ParticipantStatus.ACCEPTED
        )

        # Verify attendee participant
        self.assertEqual(
            attendee_participant.role, MeetingParticipant.ParticipantRole.ATTENDEE
        )
        self.assertEqual(
            attendee_participant.status, MeetingParticipant.ParticipantStatus.INVITED
        )

        # Verify status update
        self.assertEqual(
            created_update.update_type, MeetingStatusUpdate.UpdateType.CREATED
        )
...
class IntelligentMeetingServiceTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.meeting_tomorrow = MeetingSession.objects.create(
            ...
            scheduled_start_time=tomorrow.replace(
                hour=14, minute=0, second=0, microsecond=0
            ),
            scheduled_end_time=tomorrow.replace(
        ...
        self.meeting_next_week = MeetingSession.objects.create(
            ...
            scheduled_start_time=next_week.replace(
                hour=9, minute=0, second=0, microsecond=0
            ),
            scheduled_end_time=next_week.replace(
                hour=10, minute=0, second=0, microsecond=0
            ),
            status=MeetingSession.Status.SCHEDULED,
        )
...
class NIAMeetingSchedulerTestCase(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(
            username="niauser", email="niauser@example.com", password="testpass123"
        )
    ...
    def test_schedule_nia_meeting(self):
        ...
        with patch.object(
            self.nia_scheduler.google_service, "create_meeting"
        ) as mock_create:
            ...
            meeting_session = self.nia_scheduler.schedule_nia_meeting(
                self.user, meeting_data
            )
            ...
            self.assertEqual(
                nia_update.metadata["nia_meeting_type"], "general_consultation"
            )
    ...
    def test_calculate_preparation_time(self):
        ...
        lead_prep = self.nia_scheduler._calculate_preparation_time(
            NIAMeetingType.LEAD_CONSULTATION
        )
        training_prep = self.nia_scheduler._calculate_preparation_time(
    ...
    def test_is_optimal_nia_time(self):
        ...
        weekday_business = timezone.now().replace(
            hour=10, minute=0, second=0, microsecond=0
        )
    ...
    def test_calculate_meeting_effectiveness(self):
        ...
        effectiveness = self.nia_scheduler._calculate_meeting_effectiveness(
            good_meeting
        )
