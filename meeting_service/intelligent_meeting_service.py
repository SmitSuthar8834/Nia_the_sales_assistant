...
class IntelligentMeetingService:
    ...
    def analyze_user_availability(self, user: User, date_range: Tuple[datetime, datetime]) -> Dict:
        ...
        try:
            ...
            avg_duration = sum(duration_stats) / max(len(duration_stats), 1) if duration_stats else 60
            
            # Find available time slots
            available_slots = self._find_available_slots(user, start_date, end_date, meetings)
    ...
    def _find_available_slots(self, user: User, start_date: datetime, end_date: datetime, 
                            existing_meetings: List[MeetingSession]) -> List[Dict]:
        ...
        current_time = start_date.replace(hour=self.preferred_meeting_hours[0], minute=0, second=0, microsecond=0)
        end_time = end_date.replace(hour=self.preferred_meeting_hours[1], minute=0, second=0, microsecond=0)
        
        while current_time < end_time:
            # Check if current time is during business hours
            if self.preferred_meeting_hours[0] <= current_time.hour < self.preferred_meeting_hours[1]:
    ...
    def recommend_meeting_time(self, user: User, duration_minutes: int = 60, 
                             preferred_date: datetime = None) -> List[Dict]:
        ...
        try:
            ...
            availability = self.analyze_user_availability(user, (start_range, end_range))
            ...
            for slot in availability.get('available_slots', []):
                ...
                if self.preferred_meeting_hours[0] <= slot_hour < self.preferred_meeting_hours[1]:
                    score += 20
                ...
                recommendations.append({
                    ...
                    'score': score,
                    'reason': self._generate_recommendation_reason(slot, peak_hours, daily_count)
                })
    ...
    def _generate_recommendation_reason(self, slot: Dict, peak_hours: List[int], daily_count: int) -> str:
    ...
    def detect_meeting_conflicts(self, user: User, proposed_meeting: Dict) -> List[Dict]:
        ...
        try:
            ...
            conflicts = MeetingSession.objects.filter(
                ...
                status__in=[MeetingSession.Status.SCHEDULED, MeetingSession.Status.ACTIVE]
            ).distinct()
    ...
    def suggest_reschedule_options(self, user: User, conflicted_meeting: MeetingSession) -> List[Dict]:
        ...
        try:
            ...
            for rec in recommendations:
                time_diff = abs((rec['start_time'] - original_time).total_seconds() / 3600)
    ...
    def schedule_post_meeting_follow_up(self, meeting: MeetingSession) -> bool:
        ...
        try:
            ...
            follow_up_time = meeting.scheduled_end_time + timedelta(minutes=self.post_meeting_delay_minutes)
            ...
            if actual_end > meeting.scheduled_end_time:
                late_minutes = int((actual_end - meeting.scheduled_end_time).total_seconds() / 60)
                if late_minutes > self.late_meeting_threshold_minutes:
                    # Adjust follow-up time for late meetings
                    follow_up_time = actual_end + timedelta(minutes=self.post_meeting_delay_minutes)
            ...
            logger.info(f"Scheduled follow-up for meeting {meeting.id} at {follow_up_time}")
            return True
    ...
    def analyze_meeting_patterns(self, user: User, days_back: int = 30) -> Dict:
        ...
        try:
            ...
            patterns['most_productive_hours'] = [hour for hour, score in productive_hours[:3]]
        ...
        if participants:
            ...
            for participant in participants:
                if participant.participation_duration_minutes > 0:
                    engagement_ratio = participant.participation_duration_minutes / duration
    ...
    def _generate_pattern_recommendations(self, patterns: Dict) -> List[str]:
        ...
        if avg_duration > 90:
            recommendations.append("Consider shorter meetings - your average is longer than optimal")
        elif avg_duration < 30:
            recommendations.append("Your meetings might benefit from slightly longer durations for better outcomes")
        
        # Frequency recommendations
        total_meetings = patterns.get('total_meetings', 0)
        if total_meetings > 60:  # More than 2 per day on average
            recommendations.append("High meeting frequency detected - consider consolidating or declining some meetings")
        
        # Late meetings
        late_count = patterns.get('late_meetings_count', 0)
        if late_count > total_meetings * 0.2:  # More than 20% late
            recommendations.append("Consider adding buffer time between meetings to reduce delays")
        
        # Productive hours
        productive_hours = patterns.get('most_productive_hours', [])
        if productive_hours:
            hours_str = ', '.join([f"{h}:00" for h in productive_hours])
            recommendations.append(f"Schedule important meetings during your most productive hours: {hours_str}")
        
        # Meeting type balance
        types_dist = patterns.get('meeting_types_distribution', {})
        if len(types_dist) == 1:
            recommendations.append("Consider diversifying meeting platforms for different types of interactions")
    ...
    def get_calendar_sync_status(self, user: User) -> Dict:
        ...
        try:
            ...
            try:
                ...
                status['outlook_calendar'] = status['teams_calendar']  # Same credentials
            except MicrosoftTeamsCredentials.DoesNotExist:
                pass
            
            # Get last sync time from most recent meeting
            last_meeting = MeetingSession.objects.filter(
                Q(organizer=user) | Q(participants__user=user)
