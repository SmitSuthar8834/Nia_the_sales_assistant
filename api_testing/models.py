...
class APITestCollection(models.Model):
    ...
    name = models.CharField(max_length=255, help_text="Collection name (e.g., 'AI Core Tests')")
    description = models.TextField(blank=True, help_text="Description of the test collection")
    created_by = models.ForeignKey(User, on_delete=models.CASCADE, related_name='test_collections')
    is_ai_core = models.BooleanField(default=False, help_text="Mark as AI core functionality test")
    is_public = models.BooleanField(default=False, help_text="Make collection available to all users")
...
class APITestCase(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    collection = models.ForeignKey(APITestCollection, on_delete=models.CASCADE, related_name='test_cases')
    ...
    request_body = models.TextField(blank=True, help_text="Request body (JSON, XML, etc.)")
    ...
    expected_response_schema = models.JSONField(default=dict, blank=True, help_text="Expected response schema")
    response_assertions = models.JSONField(default=list, help_text="Response validation rules")
    
    # Execution Order and Dependencies
    execution_order = models.IntegerField(default=0, help_text="Order of execution in collection")
    ...
    extract_data = models.JSONField(default=dict, help_text="Data to extract from response for next calls")
    
    # Metadata
    is_active = models.BooleanField(default=True, help_text="Whether this test is active")
...
class APITestExecution(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    test_case = models.ForeignKey(APITestCase, on_delete=models.CASCADE, related_name='executions')
    ...
    response_headers = models.JSONField(default=dict, help_text="Response headers received")
    ...
    extracted_data = models.JSONField(default=dict, help_text="Data extracted from response")
    
    # Validation Results
    assertion_results = models.JSONField(default=list, help_text="Results of response assertions")
...
class APIAuthFlow(models.Model):
    ...
    flow_type = models.CharField(max_length=20, choices=FlowType.choices, default=FlowType.SIMPLE)
    
    # Authentication Steps Configuration
    auth_steps = models.JSONField(default=list, help_text="Ordered list of authentication steps")
    
    # Token/Cookie Storage Configuration
    token_extraction = models.JSONField(default=dict, help_text="How to extract tokens/cookies from responses")
    token_storage = models.JSONField(default=dict, help_text="Where to store extracted tokens")
    
    # Validation Configuration
    success_indicators = models.JSONField(default=list, help_text="How to determine if auth was successful")
    
    # Usage Configuration
    token_lifetime_minutes = models.IntegerField(default=60, help_text="Expected token lifetime")
    auto_refresh = models.BooleanField(default=False, help_text="Auto-refresh expired tokens")
    
    created_by = models.ForeignKey(User, on_delete=models.CASCADE, related_name='auth_flows')
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class APITestSession(models.Model):
    ...
    collection = models.ForeignKey(APITestCollection, on_delete=models.CASCADE, related_name='sessions')
    created_by = models.ForeignKey(User, on_delete=models.CASCADE, related_name='test_sessions')
    
    # Session State
    status = models.CharField(max_length=20, choices=Status.choices, default=Status.ACTIVE)
    session_data = models.JSONField(default=dict, help_text="Session variables and extracted data")
    ...
    auth_flow = models.ForeignKey(APIAuthFlow, on_delete=models.SET_NULL, null=True, blank=True)
...
class AITestScenario(models.Model):
    ...
    class ScenarioType(models.TextChoices):
        ...
        OPPORTUNITY_INTELLIGENCE = 'opportunity_intelligence', 'Opportunity Conversion Intelligence'
    ...
    description = models.TextField(help_text="Scenario description and expected behavior")
    
    # Test Data
    test_data = models.JSONField(default=dict, help_text="Sample input data for testing")
    expected_outputs = models.JSONField(default=dict, help_text="Expected AI outputs")
    validation_rules = models.JSONField(default=list, help_text="Rules to validate AI responses")
    
    # Performance Expectations
    max_response_time_ms = models.IntegerField(default=5000, help_text="Maximum acceptable response time")
    min_confidence_score = models.FloatField(default=0.7, help_text="Minimum acceptable confidence score")
...
class APIPerformanceMetric(models.Model):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    test_execution = models.OneToOneField(APITestExecution, on_delete=models.CASCADE, related_name='performance')
