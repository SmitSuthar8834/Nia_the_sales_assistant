...
def test_sales_strategy_generation():
    ...
    with patch('ai_service.services.genai.GenerativeModel') as mock_model:
        ...
        mock_response.text = json.dumps({
            ...
            "risk_mitigation": ["Identify decision makers", "Understand procurement"]
        })
        mock_model.return_value.generate_content.return_value = mock_response
...
def test_industry_insights():
    ...
    with patch('ai_service.services.genai.GenerativeModel') as mock_model:
        mock_response = MagicMock()
        mock_response.text = json.dumps({
            "industry_trends": ["Digital health transformation", "Regulatory compliance focus"],
            ...
            "sales_best_practices": ["Understand regulatory requirements", "Focus on patient outcomes"]
        })
        mock_model.return_value.generate_content.return_value = mock_response
...
def main():
    ...
    if failed == 0:
        print("🎉 All AI-powered sales recommendations functionality is working correctly!")
        return True
    else:
