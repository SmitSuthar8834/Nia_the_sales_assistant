...
from .serializers import (
    ConfigurationTemplateSerializer, IntegrationConfigurationSerializer,
    WorkflowConfigurationSerializer, ConfigurationBackupSerializer,
    RolePermissionSerializer, ConfigurationAuditLogSerializer,
    ConfigurationTestSerializer, DynamicFormSerializer, ConfigurationValidationSerializer
)
from .services_minimal import (
    ConfigurationTestService, ConfigurationBackupService,
    PermissionService, AuditService, TemplateService
)
...
class ConfigurationTemplateViewSet(viewsets.ModelViewSet):
    ...
    def use_template(self, request, pk=None):
        ...
        serializer = IntegrationConfigurationSerializer(data=config_data, context={'request': request})
        if serializer.is_valid():
            config = serializer.save()
            AuditService.log_action(request.user, 'create', config, {'template_used': template.name})
            return Response(IntegrationConfigurationSerializer(config).data, status=status.HTTP_201_CREATED)
...
class IntegrationConfigurationViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if not PermissionService.has_permission(self.request.user, 'view', 'integration'):
    ...
    def perform_create(self, serializer):
        config = serializer.save()
        AuditService.log_action(self.request.user, 'create', config, serializer.validated_data)
    
    def perform_update(self, serializer):
        old_values = {field: getattr(serializer.instance, field) 
                     for field in serializer.validated_data.keys()}
        config = serializer.save()
        AuditService.log_action(self.request.user, 'update', config, 
                               serializer.validated_data, old_values)
    ...
    def test_configuration(self, request, pk=None):
        ...
        # Check permissions
        if not PermissionService.has_permission(request.user, 'test', 'integration', config):
        ...
        try:
            ...
            AuditService.log_action(request.user, 'test', config, {'test_type': test_type})
...
class WorkflowConfigurationViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        AuditService.log_action(self.request.user, 'create', workflow, serializer.validated_data)
    
    def perform_update(self, serializer):
        old_values = {field: getattr(serializer.instance, field) 
                     for field in serializer.validated_data.keys()}
        workflow = serializer.save()
        AuditService.log_action(self.request.user, 'update', workflow, 
                               serializer.validated_data, old_values)
    ...
    def execute(self, request, pk=None):
        ...
        try:
            ...
            AuditService.log_action(request.user, 'execute', workflow, {'manual_execution': True})
...
class ConfigurationBackupViewSet(viewsets.ModelViewSet):
    ...
    def create_backup(self, request):
        ...
        try:
            ...
            AuditService.log_action(request.user, 'backup', backup, {'backup_type': backup_type})
            return Response(ConfigurationBackupSerializer(backup).data, status=status.HTTP_201_CREATED)
        except Exception as e:
            return Response({'error': str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def restore(self, request, pk=None):
        ...
        if not PermissionService.has_permission(request.user, 'restore', 'backup', backup):
            return Response({'error': 'Permission denied'}, status=status.HTTP_403_FORBIDDEN)
...
class ConfigurationValidationAPIView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            # Perform validation
            validation_result = self._validate_configuration(config_type, config_data, credentials)
    ...
    def _get_suggestions(self, config_type, config_data):
        ...
        if config_type.startswith('crm_'):
            ...
            if not config_data.get('retry_attempts'):
                suggestions.append("Consider setting retry attempts for failed API calls")
...
class AdminDashboardView(APIView):
    ...
    def get(self, request):
        ...
        stats = {
            ...
            'active_integrations': IntegrationConfiguration.objects.filter(status='active').count(),
            'total_workflows': WorkflowConfiguration.objects.count(),
            'active_workflows': WorkflowConfiguration.objects.filter(is_active=True).count(),
            'total_templates': ConfigurationTemplate.objects.filter(is_active=True).count(),
            'recent_tests': ConfigurationTest.objects.count(),
        }
        
        # Get recent activity
        recent_logs = ConfigurationAuditLog.objects.select_related('user').order_by('-timestamp')[:10]
    ...
    def _get_health_status(self):
        ...
        healthy_configs = IntegrationConfiguration.objects.filter(health_status='healthy').count()
    ...
    def _get_user_permissions(self, user):
        ...
        permission_types = ['view', 'create', 'edit', 'delete', 'test', 'backup', 'restore']
