...
class Command(BaseCommand):
        ...
        for i, template_data in enumerate(templates):
            ...
            try:
                template, created = ConfigurationTemplate.objects.get_or_create(
                    ...
                    defaults={**template_data, "created_by": user, "is_official": True},
                )
                ...
                self.stdout.write(
                    self.style.ERROR(
                        f'Error creating template {template_data["name"]}: {str(e)}'
                    )
                )
    ...
    def create_sample_integrations(self, user):
        ...
        # Get templates
        salesforce_template = ConfigurationTemplate.objects.filter(
            name="Salesforce CRM Integration"
        ).first()
        hubspot_template = ConfigurationTemplate.objects.filter(
            name="HubSpot CRM Integration"
        ).first()
        google_meet_template = ConfigurationTemplate.objects.filter(
            name="Google Meet Integration"
        ).first()
        teams_template = ConfigurationTemplate.objects.filter(
            name="Microsoft Teams Integration"
        ).first()
        gemini_template = ConfigurationTemplate.objects.filter(
            name="Gemini AI Integration"
        ).first()
        ...
        for integration_data in sample_integrations:
            ...
            if created:
                ...
                self.stdout.write(
                    f"Sample integration already exists: {integration.name}"
                )
