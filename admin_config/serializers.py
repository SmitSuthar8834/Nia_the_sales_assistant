...
class ConfigurationTestSerializer(serializers.ModelSerializer):
    ...
    triggered_by_name = serializers.CharField(
        source="triggered_by.get_full_name", read_only=True
    )
...
class ConfigurationValidationSerializer(serializers.Serializer):
    ...
    configuration_type = serializers.ChoiceField(
        choices=IntegrationConfiguration.INTEGRATION_TYPES
    )
        ...
        if config_type.startswith("crm_"):
            ...
            for field in required_fields:
                if field not in config_data:
                    raise serializers.ValidationError(
                        f"CRM configuration must contain '{field}' field"
                    )
            ...
            for field in required_fields:
                if field not in config_data:
                    raise serializers.ValidationError(
                        f"Meeting configuration must contain '{field}' field"
                    )
